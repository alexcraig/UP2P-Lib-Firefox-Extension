...
Zotero.Translate.Sandbox = {
	...
	"Export":{
		...
		/**
		 * Moves an exported attachment to a specified sub-directory of the export directory, and 
		 * optionally adds a numeric prefix to the filename if another file of the same name already 
		 * exists. Note that this assumes the specified attachment will be in the default export 
		 * location (has not been previously moved or deleted).
		 * @param {Zotero.Translate} translate
		 * @param {String} attachment The serialized attachment object, as exposed to the translator sandbox
		 * @param {String} attachment.itemID The item ID of the attachment (used to determine filename)
		 * @param {String[]} subdirs A list of subdirectory names which specify where the file should
		 *     be moved to. For example, to move an attachments to the "/files/test" sub-directory, 
		 *     this parameter should be ["files", "test"]. An empty list moves the file to the base 
		 *     directory of the export.
		 * @param {Boolean} autoRename Optional - If true, the moved attachment will be renamed with 
		 *                  a numeric prefix if a duplicate filename is encountered. If false, an 
		 *                  error will be thrown if a duplicate filename is encountered. This argument
		 *                  defaults to true if it is not provided.
		 * @return {String} The filename of the moved file, after any renaming occurs.
		 */
		"moveExportedAttachment":function(translate, attachment, subdirs, autoRename) {
			// Set the default value of the autoRename argument
			if (autoRename === undefined) {
				autoRename = true;
			}
				
			if(typeof translate.location == "object") {
				// Translator location is bound to a file
				// (Should always be the case for export translators)
				
				// For now, read from the database to get the attachment filename
				// (Not included with the serialized attachment?)
				var origFilename = Zotero.Items.get(attachment.itemID).getFile().leafName;
				var attachFile = translate.location.parent.clone();
				attachFile.append("files");
				attachFile.append(attachment.itemID);
				attachFile.append(origFilename);
				if(!attachFile.exists()) {
					throw new Error("moveExportedAttachment: Specified attachment has already been "
							+ " moved or deleted.");
				}
				var copyDir = translate.location.parent.clone();
				
				// Navigate to the specified sub-directory, and create it if necessary
				for(var i = 0; i < subdirs.length; i++) {
					copyDir.append(subdirs[i]);
				}
				if(!copyDir.exists()) {
					copyDir.create(Components.interfaces.nsIFile.DIRECTORY_TYPE, 0777);
				}
				
				// Check for duplicate filename, and rename the moved file if a duplicate exists
				var renameCounter = 2;
				var newFilename = origFilename;
				var copyFile = copyDir.clone();
				copyFile.append(newFilename);
				
				while(copyFile.exists()) {
					if(autoRename === true) {
						newFilename = "[" + renameCounter + "]" + origFilename;
						copyFile = copyDir.clone();
						copyFile.append(newFilename);
					} else {
						throw new Error("moveExportedAttachment: Found existing file with name: "
							+ origFilename + " (auto rename disabled).");
					}
				}				
				
				attachFile.moveTo(copyDir, newFilename);
				return newFilename;
			} else {
				// Translator is bound to a URL
				throw new Error("moveExportedAttachment: Called with invalid translator location set.");
			}
		},
		
		/**
		 * Deletes a specified exported attachment. Note that this assumes the specified attachment 
		 * will be in the default export location (has not been previously moved or deleted).
		 * @param {Zotero.Translate} translate
		 * @param {String} attachment The serialized attachment object, as exposed to the translator sandbox
		 * @param {String} attachment.itemID The item ID of the attachment (used to determine filename)
		 */
		"deleteExportedAttachment":function(translate, attachment) {
			if(typeof translate.location == "object") {
				// Translator location is bound to a file
				// (Should always be the case for export translators)
				...
				attachFile.append(origFilename);
				if(!attachFile.exists()) {
					throw new Error("deleteExportedAttachment: Specified attachment has already been "
							+ " moved or deleted.");
				}
				attachFile.remove(false);
			} else {
				// Translator is bound to a URL
				throw new Error("deleteExportedAttachment: Called with invalid translator location set.");
			}
		},
		
		/**
		 * Removes all files in the export "files" subdirectory. This should typically
		 * be called after moving relevant files to the base directory.
		 * @param {Zotero.Translate} translate
		 */
		"clearAttachmentDir":function(translate) {
			if(typeof translate.location == "object") {
				// Translator location is bound to a file
				// (Should always be the case for export translators)
				var filesDir = translate.location.parent;
				filesDir.append("files");
				filesDir.remove(true);
			} else {
				// Translator is bound to a URL
				throw new Error("clearAttachmentDir: Called with invalid translator location set.");
			}
		}
	},
