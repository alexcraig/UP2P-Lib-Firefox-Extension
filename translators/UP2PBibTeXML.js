{
	...
	"inRepository":false,
	"lastUpdated":"2011-06-17 14:40:00"
}

// Note, translatorType has been changed to 2 (export) until import functionality is
// better supported. Remember to change this back to 3 (1 - import + 2 export) once
// import functionality has been included
...
function storeField(field, value, fieldMap, mapKeys, isMacro) {
...
function doExport() {
	...
	while(item = Zotero.nextItem()) {
		...
		if(item.creators && item.creators.length) {
			...
			if(author) {
				storeField("author", author.substr(5), itemFieldsMap, mapKeys);
			}
			if(editor) {
				storeField("editor", editor.substr(5), itemFieldsMap, mapKeys);
			}
			if(translator) {
				storeField("translator", translator.substr(5), itemFieldsMap, mapKeys);
			}
		}
		...
		if(item.date) {
			...
			if(typeof date.month == "number") {
				storeField("month", months[date.month], itemFieldsMap, mapKeys, true);
			}
			if(date.year) {
		...
		if(Zotero.getOption("exportFileData")) {
			if(item.attachments) {
				for each(var attachment in item.attachments) {
					if((attachment.path.indexOf(".pdf", attachment.path.length - ".pdf".length) !== -1)) {
						storeField("file", "file:" + Zotero.moveExportedAttachment(attachment, []), 
								itemFieldsMap, mapKeys);
					} else {
						Zotero.deleteExportedAttachment(attachment);
					}
				}
			}
		}
